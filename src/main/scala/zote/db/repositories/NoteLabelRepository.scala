...
import zote.db.model.{NoteLabelEntity, NotePersonEntity}
...
case class NoteLabelRepositoryImpl(
  ...
  override def findAllByLabelId(labelId: Long): Task[List[NoteLabelEntity]] = transaction {
    run(query[NoteLabelEntity].filter(nl => nl.labelId == lift(labelId)))
  }

  override def insert(noteLabelEntities: Seq[NoteLabelEntity]): Task[Unit] = transaction {
    run(liftQuery(noteLabelEntities).foreach(nl => query[NoteLabelEntity].insertValue(nl))).unit
  }

  override def delete(noteLabelEntities: Seq[NoteLabelEntity]): Task[Unit] = transaction {
    run {
      liftQuery(noteLabelEntities).foreach { noteLabelEntity =>
        query[NoteLabelEntity]
          .filter(nl => nl.noteId == noteLabelEntity.noteId && nl.labelId == noteLabelEntity.labelId)
          .delete
      }
    }.unit
  }
