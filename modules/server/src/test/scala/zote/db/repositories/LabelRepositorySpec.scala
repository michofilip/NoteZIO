    ...
    suite("LabelRepository")(
      ...
      suite("provides function 'findById' that")(
        test("returns option with LabelEntity if exists") {
          for {
            ...
            maybeLabelEntity.contains(label)
          }
        },
        test("returns empty option if not exists") {
          for {
            labelRepository  <- ZIO.service[LabelRepository]
            maybeLabelEntity <- labelRepository.findById(LabelId(-1))
          } yield assertTrue {
            maybeLabelEntity.isEmpty
          }
        },
      ),
      suite("provides function 'getById' that")(
        ...
        test("returns NotFoundException if not exists") {
          for {
            labelRepository <- ZIO.service[LabelRepository]
            result          <- labelRepository.getById(LabelId(-1)).exit
          } yield assertTrue {
      ...
      suite("provides function 'upsert' that")(
        test("inserts and returns LabelEntity if not exists") {
          for {
            labelRepository <- ZIO.service[LabelRepository]
            labelEntity     <- labelRepository.upsert(LabelEntity(name = "Red"))
          } yield assertTrue {
            !labelEntity.id.isZero &&
        ...
        test("updates and returns LabelEntity if exists") {
          for {
            label           <- DbHelper.insertLabel(LabelEntity(name = "Red"))
            labelRepository <- ZIO.service[LabelRepository]
            labelEntity     <- labelRepository.upsert(label.copy(name = "Green"))
          } yield assertTrue {
      ...
      suite("provides function 'delete' that")(
        test("deletes LabelEntity if exists") {
          for {
            label                        <- DbHelper.insertLabel(LabelEntity(name = "Red"))
            labelRepository              <- ZIO.service[LabelRepository]
            maybeLabelEntityBeforeDelete <- labelRepository.findById(label.id)
            _                            <- labelRepository.delete(label.id)
            maybeLabelEntityAfterDelete  <- labelRepository.findById(label.id)
          } yield assertTrue {
            maybeLabelEntityBeforeDelete.isDefined &&
            maybeLabelEntityAfterDelete.isEmpty
          }
        },
        test("does nothing if LabelEntity not exists") {
          for {
            labelRepository <- ZIO.service[LabelRepository]
            result          <- labelRepository.delete(LabelId(-1)).exit
          } yield assertTrue {
            result.isSuccess
          }
        },
